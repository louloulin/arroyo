...
use datafusion::logical_expr::{
    BinaryExpr, Column, Expr, Filter, Join, JoinConstraint, JoinType, LogicalPlan, Operator,
    Projection, TableScan,
};
use std::collections::HashMap;
use std::sync::Arc;

use arroyo_planner::optimizers::{join_optimizer, join_rewriter, CombinedOptimizer, Optimizer};
...
fn create_test_table_scan(table_name: &str, is_dimension: bool) -> LogicalPlan {
    ...
    LogicalPlan::TableScan(TableScan {
        ...
        projected_schema: Arc::new(
            datafusion::common::DFSchema::try_from_schema(schema.clone()).unwrap(),
        ),
        filters: vec![],
        fetch: None,
        table_schema: schema,
...
fn create_test_join(left: LogicalPlan, right: LogicalPlan, join_type: JoinType) -> LogicalPlan {
    ...
    let schema = Arc::new(
        datafusion::logical_expr::build_join_schema(left.schema(), right.schema(), &join_type)
            .unwrap(),
    );
...
fn create_test_window_join(left: LogicalPlan, right: LogicalPlan) -> LogicalPlan {
    ...
    let time_window_condition = Expr::Between(Box::new(datafusion::logical_expr::expr::Between {
        ...
        low: Box::new(Expr::BinaryExpr(BinaryExpr {
            ...
            op: Operator::Minus,
            right: Box::new(Expr::Literal(datafusion::scalar::ScalarValue::IntervalDayTime(
                Some(300000),
    ...
    let schema = Arc::new(
        datafusion::logical_expr::build_join_schema(left.schema(), right.schema(), &JoinType::Inner)
            .unwrap(),
    );
...
fn plan_contains_node_type(plan: &LogicalPlan, node_type: &str) -> bool {
    match plan {
        ...
        _ => {
            ...
            match plan {
                LogicalPlan::Join(join) => {
                    plan_contains_node_type(&join.left, node_type)
                        || plan_contains_node_type(&join.right, node_type)
                }
                LogicalPlan::Filter(filter) => plan_contains_node_type(&filter.input, node_type),
                LogicalPlan::Projection(projection) => {
                    plan_contains_node_type(&projection.input, node_type)
                }
                ...
                LogicalPlan::Union(union) => {
                    union.inputs.iter().any(|input| plan_contains_node_type(input, node_type))
                }
                LogicalPlan::CrossJoin(cross_join) => {
                    plan_contains_node_type(&cross_join.left, node_type)
                        || plan_contains_node_type(&cross_join.right, node_type)
                }
                LogicalPlan::Distinct(distinct) => {
                    plan_contains_node_type(&distinct.input, node_type)
                }
                LogicalPlan::Subquery(subquery) => {
                    plan_contains_node_type(&subquery.subquery, node_type)
                }
                LogicalPlan::SubqueryAlias(alias) => {
                    plan_contains_node_type(&alias.input, node_type)
                }
                LogicalPlan::Repartition(repartition) => {
                    plan_contains_node_type(&repartition.input, node_type)
                }
                _ => false,
            }
        }
