#[cfg(test)]
mod tests {
    use arroyo_connectors::elasticsearch::ElasticsearchConnector;
    ...
    // 这个测试需要一个运行中的 Elasticsearch 实例
    // 可以使用 Docker 启动一个测试实例：
    // docker run --name elasticsearch-test -p 9200:9200 -p 9300:9300 -e "discovery.type=single-node" -e "xpack.security.enabled=false" docker.elastic.co/elasticsearch/elasticsearch:8.5.0
    #[tokio::test]
    #[ignore] // 忽略这个测试，除非明确要运行它
    async fn test_elasticsearch_connector() {
        // 创建 Elasticsearch 连接器
        let connector = ElasticsearchConnector {};

        // 验证连接器元数据
        let metadata = connector.metadata();
        assert_eq!(metadata.id, "elasticsearch");
        assert_eq!(metadata.name, "Elasticsearch");
        ...
        match result {
            TestSourceMessage::Done { message } => {
                ...
                assert!(message.contains("Successfully connected to Elasticsearch"));
            }
