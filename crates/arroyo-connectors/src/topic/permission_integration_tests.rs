...
mod tests {
    ...
    async fn test_topic_permission_integration() {
        ...
        // 设置用户角色和权限
        // 确保权限管理器存在
        assert!(admin.permission_manager.is_some(), "Permission manager should be initialized");

        // 使用 unwrap_or_else 处理 None 情况，但我们已经确保它不是 None
        let permission_manager = admin.permission_manager.as_ref().unwrap();
