...
- [x] 优化查询编译
  - [x] 扩展 `arroyo-compiler` 模块，支持新语法 - 已实现对 Topic DDL 和消息队列 DML 语句的支持
  - [x] 改进查询优化器，支持更多优化规则 - 已实现组合优化器，支持多种优化规则
  - [ ] 实现基于成本的优化器
  - [x] 开发下推优化，减少数据传输 - 已实现谓词下推和投影下推优化
  - [x] 实现谓词下推和投影下推 - 已实现谓词下推和投影下推优化器
  - [ ] 开发自适应执行计划
  - [ ] 实现查询重写和规范化
...
- [x] 实现高级分析功能
  - [x] 开发窗口聚合函数库 - 已实现方差、标准差、百分位数、中位数、协方差、相关系数等高级窗口聚合函数
  - [x] 实现流式 JOIN 优化 - 已实现自适应 JOIN 策略选择、JOIN 重分区优化、JOIN 状态管理优化和 JOIN 谓词下推优化
