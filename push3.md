...
为了解决端口冲突和服务冗余问题，我们已经实施了以下改进：

### 9.1 问题分析

1. **双重服务问题**：Push 功能同时通过 API 服务（5115 端口）和独立的 Push 服务（默认 8000 端口）提供
2. **状态不同步**：两个服务使用不同的 `TopicManager`、`MetricsManager` 和 `MessageStore` 实例，导致状态不同步
3. **维护困难**：维护两套相似的代码增加了开发和维护成本

### 9.2 解决方案

1. **移除独立 Push 服务**：
   - [x] 移除 `PushSourceFunc` 中的 `http_server` 字段
   - [x] 修改 `start_protocol_server` 方法，不再启动独立 HTTP 服务器
   - [x] 修改 `stop` 方法，移除停止 HTTP 服务器的代码
   - [x] 保留全局消息注册表，确保消息路由正常工作

2. **统一使用 API 服务端口（5115）**：
   - [x] 确保所有 Push API 请求通过 API 服务（5115 端口）处理
   - [x] 更新文档，明确说明 Push API 的正确访问方式和端口

### 9.3 优势

1. **简化架构**：只有一个服务提供 Push 功能，简化了系统架构
2. **状态一致**：所有 Push 请求使用同一套 `TopicManager`、`MetricsManager` 和 `MessageStore` 实例，确保状态一致
3. **维护简化**：只需维护一套代码，降低了开发和维护成本
4. **用户体验改善**：用户只需记住一个端口（5115），不会因为端口混淆而困惑
...
最近的改进已经解决了端口冲突和服务冗余问题，统一使用 API 服务端口（5115）提供 Push 功能，简化了系统架构，提高了状态一致性，降低了维护成本，改善了用户体验。
